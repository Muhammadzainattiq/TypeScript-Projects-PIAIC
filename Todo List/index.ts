...
let list: string[] = ["Wake up", "Say Fajar Prayer", "Recite Quran"]
...
async function removeTodo(index:number){
    ...
    list.splice(index,1)}else{console.log("Enter valid index");
    }
}

async function updateTodo(index:number, todo:string){
    if (index>=0 && index<list.length){
        list.splice(index,1,todo)}else{console.log("Enter valid index");
        }
